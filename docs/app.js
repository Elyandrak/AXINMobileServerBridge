...
function hasConfig() { const c = loadConfig(); return !!(c.url && c.apiKey); }
...
function renderSetup() {
  ...
  app.innerHTML = `
    <div class="setup-screen">
      ...
      <p class="setup-hint">Introduce la dirección de tu servidor VS y la API key configurada en el mod.</p>
      <form id="setup-form" class="setup-form" novalidate>
        <label class="field-label">URL del servidor
          <input id="inp-url" class="field-input" type="url"
            placeholder="http://192.168.1.100:42421" value="${esc(cfg.url || '')}"
        ...
        <label class="field-label">API Key
          <input id="inp-key" class="field-input" type="password"
            placeholder="Tu clave secreta" value="${esc(cfg.apiKey || '')}"
...
async function handleSetupSubmit(e) {
  ...
  if (!url || !apiKey) { showMsg('setup-msg', 'Rellena todos los campos.', 'error'); return; }
